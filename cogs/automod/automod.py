        ...
        try:
            if existing:
                ...
                async with aiosqlite.connect(DB) as db:
                    ...
                    await db.commit()
            wh = await channel.create_webhook(name="ZenoX Automod")
            async with aiosqlite.connect(DB) as db:
    ...
    async def cf_wizard(self, ctx):
        ...
        if not has_logging:
            try:
                ...
                log_channel = await ctx.guild.create_text_channel(
                    name="zenox-automod",
                    topic="AutoMod violation logs — managed by ZenoX.",
                    overwrites=overwrites,
                )
                webhook = await log_channel.create_webhook(name="ZenoX Automod")
                async with self.get_db() as db:
        ...
        done_view.add_item(discord.ui.Container(
            discord.ui.Section(
                ...
                discord.ui.TextDisplay("-# Your server is now protected by ZenoX Automod."),
                accessory=discord.ui.Thumbnail(discord.UnfurledMediaItem(url=self.bot.user.display_avatar.url)),
            ),
            _sep(),
            discord.ui.TextDisplay(
                ...
                f"{BAR} Private logging channel `#zenox-automod` created\n"
            ),
            _sep(),
            discord.ui.TextDisplay(
