...
class AutoModEvents(commands.Cog):
    ...
    async def log_violation(self, guild, user, module, content, channel):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            try:
                ...
                embed.set_footer(text=f"Violation Case {case_id} • ZenoX AutoMod", icon_url=self.bot.user.display_avatar.url)
                webhook.send(embed=embed, username="ZenoX Automod")
            except: pass
    ...
    async def execute_punishment(self, message, module, punishment, timeout_sec):
        ...
        try:
            ...
            embed.set_footer(text="ZenoX Automod • Enforcement", icon_url=message.guild.icon.url if message.guild.icon else user.display_avatar.url)
