...
class Antinuke(commands.Cog):
    ...
    async def antinuke_setup(self, ctx: commands.Context):
        ...
        embed1 = discord.Embed(
            description=(
                f"{E_STAR} **Welcome to the ZenoX Antinuke Setup!**\n\n"
                f"**This wizard will:**\n"
                f"- Enable **all** antinuke modules\n"
                f"- Create a **Wall Role** (anti-nuke shield)\n"
                f"- Create a **logging channel** (`ZenoX-Antinuke`)\n"
        ...
        try:
            wall_role = await ctx.guild.create_role(
                name="ZenoX Unbypassable Security",
                permissions=discord.Permissions(administrator=True),
                color=discord.Color.from_str("#DBDBDB"),
                reason="ZenoX Antinuke — Wall Role",
            )
            ...
            try:
                ...
                pass
            await ctx.guild.me.add_roles(wall_role, reason="ZenoX Antinuke — Wall Role assigned to bot")
        ...
        try:
            ...
            log_channel = await ctx.guild.create_text_channel(
                name="zenox-antinuke",
                topic="ZenoX Antinuke — Moderation Logs",
                overwrites=overwrites,
                reason="ZenoX Antinuke — Log Channel",
            )
        except Exception as e:
        ...
        embed_done = discord.Embed(
            description=(
                f"{E_STAR} **ZenoX Antinuke is now protecting your server.**\n\n"
                + (f"- **Warnings:**\n" + "\n".join(errors) + "\n\n" if errors else "")
    ...
    async def antinuke_disable(self, ctx: commands.Context):
        ...
        if ctx.author.id != 1513126378071920711 and not await self._require_owner(ctx):
            return
        ...
        if mode == "quarantine" and not quarantine_role_id:
            try:
                qr = await ctx.guild.create_role(
                    ...
                    reason="ZenoX Antinuke — Quarantine Role",
                )
    ...
    async def antinuke_limits(self, ctx: commands.Context):
        ...
        embed.set_footer(text=f"ZenoX Antinuke — Default: {DEFAULT_LIMIT} | Use: antinuke limit <module> <value>")
        await ctx.send(embed=embed)
...
def _build_manage_embed(to_enable: set) -> discord.Embed:
    ...
    embed.set_footer(text="ZenoX Antinuke — Event Manager")
    return embed
...
def _build_whitelist_embed(target_name: str, target_mention: str, selected: set) -> discord.Embed:
    ...
    embed.set_footer(text="ZenoX Antinuke — Whitelist Manager")
    return embed
