...
class LoggingSystem(commands.Cog):
    ...
    async def _get_webhook(self, guild: discord.Guild, log_type: str) -> Optional[discord.Webhook]:
        ...
        try:
            ...
            for wh in webhooks:
                if wh.token and wh.name == "ZenoX Logging":
                    target_webhook = wh
                    break

            if not target_webhook:
                target_webhook = await channel.create_webhook(name="ZenoX Logging", reason=f"Setup for {log_type}")
    ...
    def safe_embed(self, title: str, description: str, color: int = EMBED_COLOR) -> discord.Embed:
        ...
        embed.set_footer(text="ZenoX Logging System")
    ...
    async def logging_setup(self, ctx):
        ...
        try:
            ...
            category = await ctx.guild.create_category("ZenoX Logging", overwrites=overwrites, reason="Logging Setup")
            ...
            await status_msg.edit(embed=self.safe_embed("Setup Complete", f"<:emoji_1769867605256:1467155817726873650> Created **ZenoX Logging** category with {len(created_channels)} channels.\nLogging is now **ENABLED**."))
            ...
            for c_id in channel_ids:
                ...
                if channel:
                    if not logging_category and channel.category:
                         if channel.category.name == "ZenoX Logging":
