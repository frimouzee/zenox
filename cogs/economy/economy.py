...
def _footer(ctx) -> str:
    return f"ZenoX - Economy • {ctx.author.name}"
...
class LeaderboardView(discord.ui.View):
    ...
    def _build_embed(self) -> discord.Embed:
        ...
        embed.set_footer(text=f"Page {self.page + 1}/{self.pages} • ZenoX - Economy")
        return embed
...
class EcoGuideView(discord.ui.View):
    ...
    def get_embed(self, category: str) -> discord.Embed:
        ...
        embed.set_author(name="ZenoX Economy Guide", icon_url=self.ctx.guild.icon.url if self.ctx.guild.icon else None)
        if category == "Getting Started":
            embed.title = "🪙 Getting Started"
            embed.description = (
                "Welcome to the **ZenoX Economy**! Your journey to unimaginable wealth starts right here.\n\n"
...
class EconomyCog(commands.Cog):
    ...
    async def beg(self, ctx):
        ...
        embed.set_footer(text=f"Wallet: {_fmt(new_wallet)} coins • ZenoX - Economy")
        await ctx.reply(embed=embed, mention_author=False)
    ...
    async def pay(self, ctx, member: discord.Member = None, amount: int = None):
        ...
        embed.set_author(name=ctx.author.display_name, icon_url=ctx.author.display_avatar.url)
        embed.set_footer(text="ZenoX - Economy")
        await ctx.reply(embed=embed, mention_author=False)
