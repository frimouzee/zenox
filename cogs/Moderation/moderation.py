...
from core import Cog, ZenoX, Context
      ...
      for role_name in raw_roles:
          ...
          try:
              if target_role not in member.roles:
                  await member.add_roles(target_role, reason=f"ZenoX • Addrole | Requested By {ctx.author} (ID: {ctx.author.id})")
                  added_roles.append(target_role.name)
              else:
                  await member.remove_roles(target_role, reason=f"ZenoX • Removerole | Requested By {ctx.author} (ID: {ctx.author.id})")
